		...
		if songFileName != ""{
			...
			do {
				...
				if FileManager.default.fileExists(atPath: pathToTrack) {
					...
					CoreDataManager.instance.setLogRecord(eventDescription: "Файл для копирования во временную директорию отсутвует в исходной директории, id = \(song.trackID.description)", isError: true, errorMessage: "")
					CoreDataManager.instance.saveContext()
				}
			} catch {
